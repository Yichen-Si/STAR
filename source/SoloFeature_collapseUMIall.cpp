...
void SoloFeature::collapseUMIall()
{
    ...
    for (uint64 icb=0; icb<nCB; icb++) {//main collapse cycle
...
if (nReadPerCB[icb] <= 0) {
    ...
    if (!pSolo.multiMap.yes.multi) {
        readFeatSum->stats.V[readFeatSum->stats.yesWLmatch] = readFeatSum->stats.V[readFeatSum->stats.yessubWLmatch_UniqueFeature ];
    }
};

void SoloFeature::collapseUMIperCB(uint64 iCB, vector<uint32> &umiArray, vector<uint32> &gID,  vector<uint32> &gReadS)
{

    uint32 rN=nReadPerCB[iCB]; //the number of all aligns, not reads
    if (rN == 0) {
        countCellGeneUMIindex[iCB+1]=countCellGeneUMIindex[iCB];
        nGenePerCB[iCB]=0;
        nUMIperCB[iCB]=0;
    ...
    uint32 *rGU=rCBp[iCB]; //points to the storage of the icb-th barcode
