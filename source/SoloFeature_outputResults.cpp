...
void SoloFeature::outputResults(bool cellFilterYes, string outputPrefixMat)
{
    ...
    */
    ...
    if (pSolo.type==pSolo.SoloTypes::SeqScope) {
        nCells = nCB;
        for (auto kv : cbWL->val_idx_map) {
            if (nReadPerCB[kv.second] == 0) {
                continue;
            }
            cbStr << kv.second << '\t' << (int32_t) (kv.first >> 32) << '\t' << (int32_t) (kv.first & 0xFFFFFFFF) << '\t' << nUMIperCB[kv.second] << '\t' << nReadPerCB[kv.second] << '\n';
            nCellGeneEntries += nGenePerCB[kv.second];
        }
    } else if (cellFilterYes) {//filtered cells
        nCells = filteredCells.nCells;
    ...
    for (uint32 iCol=1; iCol<countMatStride; iCol++) {
        ...
        countMatrixStream <<"%\n";
        countMatrixStream << featuresNumber <<' '<< nCells <<' '<< nCellGeneEntries << '\n';

        uint32  cbInd1=0;
        for (uint64 icb=0; icb<nCB; icb++) {
            if (cellFilterYes) {
                if (filteredCells.filtVecBool[icb]) {
                    ++cbInd1;
                } else {
