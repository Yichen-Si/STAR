...
void SoloReadFeature::record(SoloReadBarcode &soloBar, uint nTr, Transcript **alignOut, uint64 iRead, ReadAnnotations &readAnnot)
{
    ...
    if (pSolo.readStatsYes[featureType]) {//readFlag
        ...
        if (soloBar.cbMatch<0 && (pSolo.cbWLyes || pSolo.type == pSolo.SoloTypes::SeqScope)) {//no CB match in the WL
            ...
            case SoloFeatureTypes::Velocyto :
                ...
                if (readAnnot.trVelocytoType.size()>0) {//otherwise, no gene
                    ...
                    if (pSolo.type == pSolo.SoloTypes::SeqScope) {
                        *streamReads << soloBar.umiB << ' ' << soloBar.cbMatchString << ' ' << readAnnot.trVelocytoType.size();
                    } else {
...
uint32 outputReadCB(fstream *streamOut, const uint64 iRead,
                    const int32 featureType, SoloReadBarcode &soloBar,
