...
void Transcriptome::classifyAlign (Transcript **alignG, uint64 nAlignG, ReadAnnotations &readAnnot)
{
    ...
    bool ovlpTranscriptome=false;
    bool ovlpIntron=false;
    ...
    if (!ovlpTranscriptome) {
        annFeat.ovType = ReadAnnotFeature::overlapTypes::intergenic;
    }
    if (ovlpIntron) { // ovlp with any intron, and not purely exonic for any transcript
        annFeat.ovType = ReadAnnotFeature::overlapTypes::intronic;
    }
    if ( annFeat.fSet.size()>0 ) { // purely exon for at least one transcript
