...
void build_patterns();
int32_t query_unambig_besthit_(uint64_t bcd, uint64_t& cb, T& val, int32_t max_mismatch);
...
BcdMatch() {}
BcdMatch(uint32_t _bcd_len, uint32_t _k, bool _exact = false, bool _ar = true, bool _aq = true);
void init(uint32_t _bcd_len, uint32_t _k, bool _exact = false, bool _ar = true, bool _aq = true);
void reset();
void process_ambig_ref();
void add_keys(uint64_t bcd);
int32_t add_ref(const char* seq, T& val);
int32_t query(const char* seq, uint64_t& cb, T& val, int32_t max_mismatch = 1);
