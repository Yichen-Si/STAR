...
{//velocyto counting gets info from Gene counting
        ...
        while (*streamReads >> iread) {//until the end of file
            if (pSolo.type==pSolo.SoloTypes::SeqScope) {
                ...
                *streamReads >> cb;
                ...
                cb=soloFeatAll[pSolo.featureInd[SoloFeatureTypes::Gene]]->readInfo[iread].cb;
                umi=soloFeatAll[pSolo.featureInd[SoloFeatureTypes::Gene]]->readInfo[iread].umi;
    ...
    *P.inOut->logStdOut << timeMonthDayTime(rawTime) << " ... Velocyto counting: finished input" <<endl;
    ...
    time(&rawTime);
    *P.inOut->logStdOut << timeMonthDayTime(rawTime) << " ... Velocyto counting: finished collapsing UMIs" <<endl;
